...
func loadSubtitles(assets *formats.Assets) ([]*ebiten.Image, error) {
	...
	ret = append(ret, subtitleSprite.GetImageRegion(0, 0, 320, 32).GetEbitenImage())    // We the lord of waterdeep...
	ret = append(ret, subtitleSprite.GetImageRegion(0, 32, 320, 67).GetEbitenImage())   // Give call to the heroes of the land...
	ret = append(ret, subtitleSprite.GetImageRegion(0, 67, 320, 80).GetEbitenImage())   // Master..
	ret = append(ret, subtitleSprite.GetImageRegion(0, 80, 320, 96).GetEbitenImage())   // They think they have found...
	ret = append(ret, subtitleSprite.GetImageRegion(0, 96, 320, 128).GetEbitenImage())  // We commission you to find...
	ret = append(ret, subtitleSprite.GetImageRegion(0, 128, 320, 144).GetEbitenImage()) // Prepare for the ...
	ret = append(ret, subtitleSprite.GetImageRegion(0, 144, 320, 160).GetEbitenImage()) // Begin your search below ...
	ret = append(ret, subtitleSprite.GetImageRegion(0, 160, 320, 175).GetEbitenImage()) // We have them...
	ret = append(ret, subtitleSprite.GetImageRegion(0, 175, 320, 191).GetEbitenImage()) // Their fate is sealed...
